...
Writer-specific companion to `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/blog-delivery-contract.md`.

## Gate Steps

1. Capability discovery and hero:
   - Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py --draft <folder> --gate 1`.
   - If a valid local hero already exists, keep it.
   - If generation is needed, use `blog-image` or `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/generate_hero.py --topic "<title>" --tags "<tags>" --out <folder>`.
   - Record provider, model ID, prompt, license, and attribution.
2. Format completeness:
   - Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/blog_render.py --md <slug>.md --out-dir <folder>`.
...
4. Visual and asset gates:
   - Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py --draft <folder> --strict`.
