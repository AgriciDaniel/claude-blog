...
This contract is the v1.9.0 answer to a failure pattern from the v1.8.x cycle: skills had reviewers, but the reviewer ran as advisory and the writer presented sloppy drafts anyway. The fix is infrastructure, not effort. Same shape as `${CLAUDE_PLUGIN_ROOT}/scripts/lint_prose.py` (v1.8.4) and `tests/test_installer_sync.py` (v1.8.6).

## Gate summary

| Gate | What it enforces | Failure mode | Implementation |
|---|---|---|---|
| 1. Capability Discovery | Required tools + agents are available before write begins | Block if no valid local hero and no permitted image path; block if reviewer agent missing | `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py --gate 1` |
| 2. Format Completeness | `.md` + `.html` + `.pdf` + `hero.(png\|jpg)` all present | Block on any missing artifact | `${CLAUDE_PLUGIN_ROOT}/scripts/blog_render.py` renders `.html` and `.pdf`; `${CLAUDE_PLUGIN_ROOT}/scripts/generate_hero.py` emits `hero.<ext>` |
| 3. Visual Verification | Rendered HTML has no SVG overflow, no console errors, valid JSON-LD | Block on any defect; preserve screenshots | `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py --gate 3` via `patchright` |
| 4. Content Review | `blog-reviewer` scores ≥ 90/100 AND zero P0 issues | Block + iterate | `agents/blog-reviewer.md` (now blocking) |
| 5. Asset + Link Integrity | Every `<img>` resolves, every `<a>` returns 200, schema validates | Block on any 404 or count mismatch | `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py --gate 5` |
...
- **Helper scripts present**: `${CLAUDE_PLUGIN_ROOT}/scripts/lint_prose.py`, `${CLAUDE_PLUGIN_ROOT}/scripts/analyze_blog.py`, the new `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py` itself.
...
Implementation split: `${CLAUDE_PLUGIN_ROOT}/scripts/blog_render.py` renders `.html` and `.pdf` from the existing `.md`; `${CLAUDE_PLUGIN_ROOT}/scripts/generate_hero.py` emits `hero.<ext>` plus `hero-credit.txt`; `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py` Gate 2 verifies all four artifacts. Failure to produce any of the four artifacts blocks delivery.

### Optional hygiene pass (non-blocking)

After rendering and before Gate 3, you may run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/blog_hygiene.py --md <slug>.md --html <slug>.html --apply` to auto-apply judgment-free fixes: add `loading="lazy"` to images that lack it and insert a Table of Contents on posts over 2000 words. It is optional, never blocks delivery, and reports any images missing alt text (which a human must fill in). It does not replace any gate.
...
Machine-readable by `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py` so the orchestrator does not have to parse the human-readable scorecard.
...
Implementation: `${CLAUDE_PLUGIN_ROOT}/scripts/generate_hero.py`. Always writes `hero-credit.txt` next to `hero.<ext>` for attribution compliance, even when generation paths 1-2 (AI-generated, no attribution needed) are used. The file then contains "AI-generated; no attribution required."
...
2. **Construct iteration prompt** keyed to the failing gate:
   - Gate 2 missing artifact → re-run `${CLAUDE_PLUGIN_ROOT}/scripts/blog_render.py` after fixing the source `.md`
   - Gate 3 visual fail → re-run `${CLAUDE_PLUGIN_ROOT}/scripts/blog_render.py` with adjusted layout (e.g. shrink SVG inner area, wrap long labels)
...
Strict mode is the default. Users can override only via an explicit operator-controlled `--no-strict` flag on `${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py` or signed/trusted project config. Draft frontmatter is untrusted content and must never disable delivery gates. Any bypass logs loudly:
...
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/quality-scoring.md`: the 100-point numeric scoring rubric used by Gate 4
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/editorial-heuristics.md`: the P0-P3 ordinal scoring used for the P0 filter in Gate 4
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/visual-media.md`: image and asset standards consumed by Gate 5
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/schema-stack.md`: JSON-LD structure validated by Gate 3 step 5
- `agents/blog-reviewer.md`: the reviewer agent that produces the Gate 4 scorecard
- `${CLAUDE_PLUGIN_ROOT}/scripts/load_untrusted_root.py`: the v1.8.3 helper used for project-root file loading in Gate 1
- `${CLAUDE_PLUGIN_ROOT}/scripts/lint_prose.py`: the v1.8.4 prose linter run as part of Gate 4's editorial-heuristics scoring
